...
class Term:
    ...
    def __repr__(self) -> str:
        ...
        if is_constant(self.root) or is_variable(self.root):
            return self.root
        return self.root + '(' + ','.join(str(argument) for argument in self.arguments) + ')'
    ...
    def substitute(self, substitution_map: Mapping[str, Term],
        ...
        if is_constant(self.root) or is_variable(self.root):
            if self.root in substitution_map:
                substituted = substitution_map[self.root]
                bad = substituted.variables() & set(forbidden_variables)
                if bad:
                    raise ForbiddenVariableError(next(iter(bad)))
                return substituted
            return self

        new_arguments = [argument.substitute(substitution_map, forbidden_variables)
                         for argument in self.arguments]
...
class Formula:
    ...
    def _parse_prefix(string: str) -> Tuple[Formula, str]:
        ...
        if is_quantifier(string[0]):
            ...
            while i < len(string) and string[i].isalnum():
                i += 1
            variable = string[1:i]
            statement, rest = Formula._parse_prefix(string[i+1:])  # skip '['
            return Formula(string[0], variable, statement), rest[1:]  # skip ']'
        ...
        if string[0] == '(':
            ...
            return Formula(root, first, second), rest[1:]  # skip ')'

        name_end = 0
        while name_end < len(string) and string[name_end].isalnum():
            name_end += 1
        name = string[:name_end]

        if name != '' and is_relation(name):
            rest = string[name_end:]
            if rest[0] == '(':
                rest = rest[1:]
                arguments = []
                if rest[0] != ')':
    ...
    def substitute(self, substitution_map: Mapping[str, Term],
        ...
        new_substitution_map = {name: term for name, term in substitution_map.items()
                                if name != self.variable}
        new_forbidden = set(forbidden_variables) | {self.variable}
        return Formula(self.root, self.variable,
                       self.statement.substitute(new_substitution_map, new_forbidden))
    ...
    def propositional_skeleton(self) -> Tuple[PropositionalFormula,
        ...
        def helper(formula):
            if is_unary(formula.root):
                return PropositionalFormula('~', helper(formula.first))
            if is_binary(formula.root):
                return PropositionalFormula(formula.root, helper(formula.first),
                                            helper(formula.second))
            ...
            return PropositionalFormula(cache[key])

        return helper(self), substitution_map
    ...
    def from_propositional_skeleton(skeleton: PropositionalFormula,
        ...
        if is_unary(skeleton.root):
            return Formula('~', Formula.from_propositional_skeleton(
